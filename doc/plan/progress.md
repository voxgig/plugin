...
| Item | Status | Notes |
|---|---|---|
| 1.0 The plan register | IN PROGRESS | This file and its four companions. §18.1 requires each phase to update the register in the same change that lands the work, so it exists before the work does. |
| 1.1 `ref` corpus section (**C1**) | NOT STARTED | Pure data. Owed to station **before its Stage 2** — earlier than this phase's exit. |
...
| 1.6 `tools/check_probes.py` | NOT STARTED | Needs 1.3 to check against; `make probes` currently reports its own absence rather than passing silently. |
