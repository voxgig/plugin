...
PROJECT = "Voxgig Plugin"

# The vocabulary directory under .vale/styles/config/vocabularies/.
VOCAB = "Plugin"

# The root pages, always required: the overview and the single
# language-neutral guide (tutorial, how-to, reference, driver contract,
# explanation, as numbered sections).
ROOT_PAGES = ("README.md", "DOCS.md")

# No documentation tree: docs/ holds the design and the decision records,
# which are working documents, and doc/plan/ is the register.
DOC_GLOBS = ()
NOT_DOCS = ()

# Pages outside the tree. None.
EXTRA_PAGES = ()
...
# the tutorial is the `## 1.` section of DOCS.md, which the shared body
# below detects by its heading, so this stays empty.
TUTORIAL_PAGES = ()
...
WORKING_DOCS = (
    ...
    (r"\bADR\.md\b", "ADR.md"),
    (r"\bdoc/plan/", "the plan register (doc/plan/)"),
    (r"\b(?:adoption|contracts|handover|progress|status)\.md\b",
     "a plan register file"),
